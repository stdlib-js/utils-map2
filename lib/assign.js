...
function map2( x, y, out, fcn, thisArg ) {
	...
	var sh;
	if ( !isFunction( fcn ) ) {
		throw new TypeError( format( '0kc3E', fcn ) );
	}
	...
	if ( isxnd ) { // note: assertion order matters here, as an ndarray-like object is also array-like
		if ( !isynd ) {
			throw new TypeError( format( '0kcC5', y ) );
		}
		if ( !isznd ) {
			throw new TypeError( format( '0kcC6', out ) );
		}
		if ( isReadOnly( out ) ) {
			throw new Error( format( '0kc1q' ) );
		}
