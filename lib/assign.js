...
function map2( x, y, out, fcn, thisArg ) {
	...
	var sh;
	if ( !isFunction( fcn ) ) {
		throw new TypeError( format( '1VX32', fcn ) );
	}
	...
	if ( isxnd ) { // note: assertion order matters here, as an ndarray-like object is also array-like
		if ( !isynd ) {
			throw new TypeError( format( '1VXBI', y ) );
		}
		if ( !isznd ) {
			throw new TypeError( format( '1VXBJ', out ) );
		}
		if ( isReadOnly( out ) ) {
			throw new Error( format('1VX1g') );
		}
